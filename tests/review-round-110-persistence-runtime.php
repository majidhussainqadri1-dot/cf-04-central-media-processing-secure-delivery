...
r110(str_contains($p,'SHOW INDEX FROM')&&str_contains($p,"$indexes['event_id']"),'schema readiness validates critical uniqueness/index invariants, not just column names');
r110(str_contains($p,'?int $expectedVersion=null')&&str_contains($l,"RecordStore::delete('provider_mapping',(string)$m['id'],(int)$m['version'])")&&str_contains($u,"RecordStore::delete('upload_part',(string)$p['id'],(int)$p['version'])"),'authoritative cleanup deletes are version-bound against concurrent mutation');
