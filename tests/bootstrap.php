...
define('SCM_VERSION','1.3.0-rc.1');
define('SCM_SCHEMA_VERSION','1.5.0');
define('SCM_CONTRACT_VERSION','1.5.0');
...
add_filter('scm_sandbox_tool_result',static function($value,string $operation,array $request): array {
    ...
    elseif($operation==='transform'){
        ...
        elseif($op==='video-pipeline'){$kinds=['hls-manifest','hls-segment','dash-manifest','poster','waveform','video-low','audio-low','caption-ref','transcript-ref'];$outs=[];foreach($kinds as $kind){$s=scm_clone_stream($source);$stats=Utils::streamHash($s);$outs[]=['kind'=>$kind,'stream'=>$s,'sha256'=>$stats['sha256'],'bitrate'=>800000,'preset_version'=>'1'];}$output=['probe'=>['supported'=>true,'duration_seconds'=>10],'outputs'=>$outs];}
...
function policy(string $media='document',string $privacy='C3',array $ops=['view','download','extract_text','ocr']): array {return Sabri\CentralMedia\Policy::normalize(['policy_id'=>'p-'.$media,'policy_version'=>1,'owner_domain'=>'file17','purpose'=>'verified-user-transfer','lawful_basis'=>'contract','revocation_hook'=>'owner-contract','privacy_class'=>$privacy,'media_class'=>$media,'max_size_bytes'=>1073741824,'max_part_size_bytes'=>8388608,'max_upload_parts'=>20000,'allowed_mime_types'=>$media==='document'?['application/pdf']:($media==='image'?['image/png']:($media==='video'?['video/mp4']:['audio/mpeg'])),'allowed_extensions'=>$media==='document'?['pdf']:($media==='image'?['png']:($media==='video'?['mp4']:['mp3'])),'required_scans'=>['hash','magic','mime','malware','archive','polyglot','decompression_bomb','metadata'],'derivative_set'=>['preview','thumbnail','text','ocr','hls','dash','poster','waveform','video-low','audio-low','caption-ref','transcript-ref'],'max_width'=>4096,'max_height'=>4096,'max_pixels'=>20000000,'max_duration_seconds'=>3600,'max_pages'=>500,'max_archive_ratio'=>100,'max_archive_depth'=>5,'max_archive_entries'=>10000,'retention'=>['class'=>'private-standard','source_seconds'=>86400,'derivative_seconds'=>86400,'temporary_seconds'=>3600,'backup_expiry_seconds'=>2592000],'rights'=>rights($ops,$privacy==='C0'),'delivery'=>['modes'=>['same_origin_proxy','token_endpoint'],'grant_ttl_seconds'=>300,'allow_ranges'=>true,'max_range_bytes'=>8388608,'allow_download'=>in_array('download',$ops,true),'public_cdn'=>$privacy==='C0','content_disposition'=>'inline'],'metadata'=>['strip_location'=>true,'strip_device'=>true,'strip_author'=>true,'preserve_orientation'=>true,'preserve_rights'=>true,'preserve_accessibility'=>true],'safety'=>['require_reviewer_for_low_confidence'=>true,'minimum_confidence'=>0.80]],true);}
