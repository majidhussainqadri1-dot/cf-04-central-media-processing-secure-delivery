...
$manifestState=<<<'PATTERN'
($m['status']??'')!=='active'
PATTERN;
$derivativeState=<<<'PATTERN'
($d['status']??'')!=='validated'
PATTERN;
...
r108(str_contains($o,$manifestState)&&str_contains($o,$derivativeState),'restore manifest reconciliation validates active ownership and derivative records instead of checking only a non-empty manifest id');
