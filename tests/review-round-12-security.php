...
$rest=file_get_contents($root.'/sabri-central-media/includes/class-scm-rest.php');$processStart=strpos($rest,'public static function processAsset');$processEnd=strpos($rest,'public static function issueGrant',$processStart);$process=substr($rest,$processStart,$processEnd-$processStart);gate(str_contains($process,'Auth::assertActor($actor)')&&str_contains($process,"'actor_id'=>$actor")&&str_contains($process,"'authorize_processing'"),'REST processing revalidates authenticated actor with canonical owner');
