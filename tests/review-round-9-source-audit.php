...
$assert(str_contains($audit,"Db::table('audit')")&&str_contains($audit,'audit_write_failed'),'audit persistence failure is represented in source');
$assert(str_contains($provider,'storage_provider_unavailable')&&str_contains($provider,'storage_provider_unhealthy'),'runtime storage fallback is fail closed when provider registration/health fails');
...
$assert(str_contains($matrix,'Complete: **0 / 33**')&&str_contains($matrix,'Partial: **20 / 33**')&&str_contains($matrix,'Missing: **13 / 33**'),'requirement matrix does not falsely claim full coded completion');
$assert(str_contains($status,'not 100% complete')&&str_contains($status,'production activation prohibited'),'runtime status preserves truthful release boundary');
