...
$q=QuotaService::reserve('file17',77,100,1,['storage_bytes'=>1000,'daily_bytes'=>1000,'jobs'=>10,'burst_bytes'=>500]);QuotaService::settle($q['quota_id'],$q['reservation_id'],true,100,1);ok((RecordStore::get('quota',$q['quota_id'])['used_bytes']??0)===100,'FR-005 atomic quota reserve/settle');QuotaService::scoreAbuse('file17',77,101,'malicious');err(fn()=>QuotaService::reserve('file17',77,1,1,['storage_bytes'=>1000,'daily_bytes'=>1000,'jobs'=>10,'burst_bytes'=>500,'abuse_block_score'=>100]),'abuse_policy_block','FR-005 abuse score blocks');
...
$hold=LegalHoldService::place($asset['id'],11,['authority'=>'Founder Security Office','reason'=>'investigation','scope'=>['source','derivatives'],'review_at'=>time()+3600,'access_restriction'=>'restricted']);err(fn()=>DeletionService::request($asset['id'],11,'user-request'),'asset_on_hold','FR-026 active hold blocks deletion');$hold=LegalHoldService::review($hold['id'],11,'release','review completed');ok($hold['status']==='released','FR-026 authorized hold review/release');
...
$queueJob=RecordStore::put('job','fair-job',['actor_id'=>0,'job_id'=>'fair-job','asset_id'=>'fair-asset','job_type'=>'probe','depends_on'=>[],'priority'=>'bulk','priority_weight'=>20,'tenant'=>'tenant-b','status'=>'queued','attempts'=>0,'max_attempts'=>3,'next_attempt_at'=>time()-1,'created_at'=>time()-3600]);$leased=JobService::lease('fair-worker',['probe']);ok($leased&&$leased['id']==='fair-job'&&$leased['lease_expires_at']>time(),'FR-028 priority, tenant fairness and starvation aging');JobService::complete($leased['id'],$leased['lease_token']);
