...
r109(str_contains($c,'streamHash($stream,?int $maxBytes=null)')&&str_contains($u,"Utils::streamHash($stream,(int)$upload['policy']['max_part_size_bytes'])"),'chunked/missing-content-length uploads are bounded during hashing rather than only after reading the full part');
