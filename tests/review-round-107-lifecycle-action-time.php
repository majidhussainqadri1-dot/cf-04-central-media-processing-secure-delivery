...
r107(str_contains($s,"'object_version'=>(int)$asset['object_version']")&&str_contains($s,"'owner_contract_version'"),'legal hold persists the canonical object/version contract it was placed against');
...
r107(str_contains($s,'DeliveryService::purgePublicForAsset')&&str_contains($s,"if((int)$purge['pending']>0)"),'destructive lifecycle refuses to advance while any public CDN purge remains pending');
