...
final class RuntimeGuard {
    ...
    public static function requireReady(array $capabilities=[]): void {
        ...
        if(function_exists('apply_filters')){
            ...
            if(!defined('SCM_TEST_MODE') && (!is_array($evidence)||($evidence['approved']??false)!==true)) throw new Error('activation_evidence_missing','Approved staging/provider/migration/rollback evidence is required.',503);
        }
    }
}
...
final class DomainRegistry {
    ...
    public static function register(string $domain,string $version,array $callbacks,array $capabilities=[]): void {
        $domain=Utils::key($domain,64);$version=Utils::text($version,32);
        if($domain===''||$version===''||$callbacks===[]) throw new Error('domain_contract_invalid','Domain contract is incomplete.',500);
        self::$domains[$domain]=['version'=>$version,'callbacks'=>$callbacks,'capabilities'=>$capabilities];
    }
...
final class Auth {
    ...
    public static function capability(string $capability): int { $id=self::requireUser();if(!function_exists('current_user_can')||!current_user_can($capability))throw new Error('capability_denied','Current actor is not authorized.',403,['capability'=>$capability]);return $id; }
...
final class RightsPolicy {
    public static function normalize(array $input): array {
        ...
        $ops=array_values(array_unique(array_map(fn($v)=>Utils::key((string)$v,32),(array)$input['allowed_operations'])));
        $aud=array_values(array_unique(array_map(fn($v)=>Utils::key((string)$v,64),(array)$input['allowed_audiences'])));
        $territories=array_values(array_unique(array_map(fn($v)=>strtoupper(Utils::text((string)$v,8)),(array)$input['allowed_territories'])));
        if($ops===[]||$aud===[]||$territories===[])throw new Error('rights_policy_incomplete','Rights policy lists cannot be empty.',400);
        ...
        $normalized['policy_hash']=hash('sha256',Utils::canonicalJson($normalized));return $normalized;
    }
...
final class Policy {
    ...
    public static function normalize(array $input,bool $strict=true): array {
        ...
        $mimes=array_values(array_unique(array_map(fn($m)=>strtolower(Utils::text((string)$m,96)),(array)$input['allowed_mime_types'])));if($mimes===[])throw new Error('mime_allowlist_empty','MIME allowlist required.',400);
        $scans=array_values(array_unique(array_map(fn($s)=>Utils::key((string)$s,48),(array)$input['required_scans'])));
        ...
        $derivatives=array_values(array_unique(array_map(fn($d)=>Utils::key((string)$d,64),(array)$input['derivative_set'])));
        ...
        $modes=array_values(array_unique(array_map(fn($m)=>Utils::key((string)$m,32),(array)$delivery['modes'])));if($modes===[])throw new Error('delivery_mode_missing','Delivery mode required.',400);
        ...
        $normalized=[
            ...
            'allowed_mime_types'=>$mimes,'allowed_extensions'=>array_values(array_unique(array_map(fn($x)=>strtolower(Utils::key((string)$x,16)),(array)($input['allowed_extensions']??[])))),
            ...
            'safety'=>['require_reviewer_for_low_confidence'=>Utils::bool($input['safety']['require_reviewer_for_low_confidence']??true),'minimum_confidence'=>(float)($input['safety']['minimum_confidence']??0.80)],
