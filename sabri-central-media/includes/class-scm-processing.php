...
final class JobService {
    ...
    public static function retryDeadLetter(string $jobId,int $actor,string $reason): array {Auth::capability('media_reprocess');Auth::assertActor($actor,'manage_options');$reason=Utils::text($reason,500);if($reason==='')throw new Error('operator_reason_required','A retry reason is required.',400);$job=RecordStore::get('job',$jobId);if(!$job||($job['status']??'')!=='dead_letter')throw new Error('dead_letter_not_found','Dead-letter job not found.',404);$asset=RecordStore::get('asset',(string)($job['asset_id']??''));$type=(string)($job['job_type']??'');if(!$asset||(int)($job['processing_generation']??0)!==(int)($asset['processing_generation']??0)||(string)($asset['job_graph'][$type]??'')!==$jobId)throw new Error('job_generation_stale','Dead-letter job is not part of the current processing generation.',409);$job['status']='queued';$job['attempts']=0;$job['next_attempt_at']=Utils::now();$job['operator_reason']=$reason;$job['operator_id']=$actor;return RecordStore::put('job',$jobId,$job,(int)$job['version']);}
