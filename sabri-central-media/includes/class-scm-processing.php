...
final class JobService {
    ...
    public static function retryDeadLetter(string $jobId,int $actor,string $reason): array {RuntimeGuard::requireReady(['streaming']);Auth::capability('media_reprocess');Auth::assertActor($actor,'manage_options');$reason=Utils::text($reason,500);if($reason==='')throw new Error('operator_reason_required','A retry reason is required.',400);$job=RecordStore::get('job',$jobId);if(!$job||($job['status']??'')!=='dead_letter')throw new Error('dead_letter_not_found','Dead-letter job not found.',404);$asset=RecordStore::get('asset',(string)($job['asset_id']??''));$type=(string)($job['job_type']??'');if(!$asset||(int)($job['processing_generation']??0)!==(int)($asset['processing_generation']??0)||(string)($asset['job_graph'][$type]??'')!==$jobId)throw new Error('job_generation_stale','Dead-letter job is not part of the current processing generation.',409);$job['status']='queued';$job['attempts']=0;$job['next_attempt_at']=Utils::now();$job['operator_reason']=$reason;$job['operator_id']=$actor;return RecordStore::put('job',$jobId,$job,(int)$job['version']);}
...
final class DerivativeService {
    ...
    $mime=self::mime($kind,$lineage,$policy);$normalizedLineage=self::lineage($lineage+['mime'=>$mime]);
    $identity=['asset_id'=>$assetId,'kind'=>$kind,'sha256'=>$stats['sha256'],'policy_hash'=>(string)$policy['policy_hash'],'job_id'=>$normalizedLineage['job_id'],'processing_generation'=>$normalizedLineage['processing_generation'],'preset'=>$normalizedLineage['preset'],'preset_version'=>$normalizedLineage['preset_version'],'mime'=>$mime];
    $id=hash('sha256',Utils::canonicalJson($identity));$existing=RecordStore::get('derivative',$id);
    if($existing){
        $same=($existing['asset_id']??'')===$assetId&&($existing['kind']??'')===$kind&&($existing['status']??'')==='validated'&&hash_equals((string)($existing['sha256']??''),$stats['sha256'])&&(int)($existing['size']??-1)===(int)$stats['size']&&Utils::canonicalJson((array)($existing['lineage']??[]))===Utils::canonicalJson($normalizedLineage);
        if(!$same)throw new Error('derivative_identity_conflict','Deterministic derivative identity conflicts with existing output.',409,['derivative_id'=>$id]);
        $provider=ProviderRegistry::get((string)($existing['storage']['provider_id']??ProviderRegistry::activeId()));if(!$provider->exists((string)$existing['object_key']))throw new Error('derivative_object_missing','Deterministic derivative record exists but its object is unavailable.',503,['derivative_id'=>$id]);
        return $existing;
    }
    $objectKey=hash('sha256','derivative|'.$id.'|'.$stats['sha256'].'|'.$policy['policy_hash']);
    ...
    $record=['actor_id'=>0,'derivative_id'=>$id,'asset_id'=>$assetId,'kind'=>$kind,'mime'=>$mime,'status'=>'validated','sha256'=>$stats['sha256'],'size'=>$stats['size'],'object_key'=>$stored['object_key'],'storage'=>$stored,'lineage'=>$normalizedLineage,'created_at'=>Utils::now(),'superseded_by'=>null];
    try{return RecordStore::put('derivative',$id,$record,0);}
    catch(Error $exception){
        if($exception->errorCode==='record_version_conflict'){$winner=RecordStore::get('derivative',$id);$same=$winner&&($winner['asset_id']??'')===$assetId&&($winner['kind']??'')===$kind&&hash_equals((string)($winner['sha256']??''),$stats['sha256'])&&(int)($winner['size']??-1)===(int)$stats['size']&&($winner['object_key']??'')===$stored['object_key'];if($same)return $winner;}
        $winner=RecordStore::get('derivative',$id);if(!$winner||($winner['object_key']??'')!==($stored['object_key']??'')){try{$store->delete((string)$stored['object_key']);}catch(\Throwable){}}
        throw $exception;
    ...
    if($activeId!==''){$activeReplay=RecordStore::get('manifest',$activeId);if($activeReplay&&($activeReplay['status']??'')==='active'&&(int)($activeReplay['processing_generation']??0)===$currentGeneration){$activeIds=array_values(array_unique(array_map(fn($item)=>(string)($item['derivative_id']??''),(array)($activeReplay['derivatives']??[]))));$requested=$derivativeIds;sort($activeIds,SORT_STRING);sort($requested,SORT_STRING);if($activeIds===$requested)return $activeReplay;}}
