...
final class DeliveryService {
    ...
    $providerId=Utils::key((string)($target['storage']['provider_id']??''),64);if($providerId==='')throw new Error('storage_provider_missing','Delivery target provider identity missing.',500);
    ...
    try{$grant['uses']=(int)$grant['uses']+1;$grant['last_used_at']=Utils::now();$grant['last_range']=$range;$grant=RecordStore::put('grant',(string)$grant['id'],$grant,(int)$grant['version']);}
    catch(\Throwable $exception){fclose($output);throw $exception;}
