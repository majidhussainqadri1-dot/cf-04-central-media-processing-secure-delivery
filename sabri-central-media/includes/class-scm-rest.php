...
final class Rest {
    ...
    public static function putPart($request): mixed {self::assertContract($request);$id=self::route($request,'id');$part=(int)self::route($request,'part');$upload=RecordStore::get('upload',$id);if(!$upload||(int)($upload['actor_id']??0)!==Auth::currentUser())return self::wrap(fn()=>throw new Error('upload_not_found','Upload session not found.',404));$max=(int)($upload['policy']['max_part_size_bytes']??0);[$stream,$close]=self::partStream($request,$max);try{return self::wrap(fn()=>self::clean(UploadService::putPart($id,Auth::currentUser(),$part,$stream,self::header($request,'x-content-sha256'),self::header($request,'x-upload-credential'))));}finally{if($close&&is_resource($stream))fclose($stream);}}
    ...
    public static function uploadAction($request): mixed {self::assertContract($request);$i=self::input($request);$id=self::route($request,'id');$actor=Auth::currentUser();$cred=self::header($request,'x-upload-credential');return self::wrap(fn()=>self::clean(match(self::route($request,'action')){'pause'=>UploadService::pause($id,$actor,$cred),'resume'=>UploadService::resume($id,$actor,$cred),'abort'=>UploadService::abort($id,$actor,$cred,(string)($i['reason']??'user-abort')),default=>throw new Error('upload_action_invalid','Invalid upload action.',400)},['policy','parts']));}
    ...
    public static function issueGrant($request): mixed {self::assertContract($request);$i=self::input($request);return self::wrap(function()use($i){$actor=Auth::currentUser();$context=(array)($i['context']??[]);$context['audience_type']='private';$service=function_exists('apply_filters')?(string)apply_filters('scm_web_delivery_service_id','web',$actor):'web';$token=DeliveryService::issue((string)($i['asset_id']??''),isset($i['derivative_id'])?(string)$i['derivative_id']:null,$actor,$service,['type'=>'user','user_id'=>$actor],$context,(string)($i['operation']??'view'),(array)($i['range_policy']??[]),(string)($i['session_id']??''),(int)($i['ttl']??300),(int)($i['max_uses']??20));return ['endpoint'=>function_exists('home_url')?home_url('/scm-media-delivery/'):'/scm-media-delivery/','token'=>$token];});}
    public static function serveMetadata($request): mixed {self::assertContract($request);$i=self::input($request);return self::wrap(function()use($i){Utils::requireFields($i,['grant_id'],'grant_id_missing');$grant=RecordStore::get('grant',(string)$i['grant_id']);if(!$grant||(int)$grant['actor_id']!==Auth::currentUser())throw new Error('grant_not_found','Grant not found.',404);return ['grant_id'=>$grant['id'],'status'=>$grant['status'],'expires_at'=>$grant['expires_at'],'endpoint'=>function_exists('home_url')?home_url('/scm-media-delivery/'):'/scm-media-delivery/'];});}
    public static function createTransfer($request): mixed {self::assertContract($request);$i=self::input($request);return self::wrap(fn()=>self::clean(TransferService::create((array)($i['transfer']??[]),(array)($i['policy']??[])),['policy']));}
    public static function listDownloads($request=null): mixed {if($request!==null)self::assertContract($request);return self::wrap(fn()=>array_map(fn($v)=>self::clean((array)$v),DownloadManagerService::list(Auth::currentUser())));}
    public static function createDownload($request): mixed {self::assertContract($request);$i=self::input($request);return self::wrap(fn()=>self::clean(DownloadManagerService::create(Auth::currentUser(),(string)($i['asset_id']??''),isset($i['derivative_id'])?(string)$i['derivative_id']:null,(string)($i['mode']??'view'),(array)($i['context']??[]))));}
    public static function requestDeletion($request): mixed {self::assertContract($request);$i=self::input($request);return self::wrap(fn()=>self::clean(DeletionService::request((string)($i['asset_id']??''),Auth::currentUser(),(string)($i['reason']??'user-request'),(array)($i['context']??[]))));}
    public static function processDeletion($request): mixed {self::assertContract($request);return self::wrap(fn()=>self::clean(DeletionService::process(self::route($request,'id'))));}
    public static function placeHold($request): mixed {self::assertContract($request);$i=self::input($request);return self::wrap(fn()=>self::clean(LegalHoldService::place((string)($i['asset_id']??''),Auth::currentUser(),(array)($i['hold']??[]))));}
