...
final class StreamingEndpoint {
    ...
    public static function handle(): void {
        ...
        try{
            ...
            exit;
        }catch(Error $e){if($started)exit;$trace=Utils::id('trace');if(function_exists('status_header'))status_header($e->httpStatus);else http_response_code($e->httpStatus);header('Content-Type: application/json; charset=utf-8');header('Cache-Control: no-store');echo Utils::json(['code'=>$e->errorCode,'message'=>$e->getMessage(),'status'=>$e->httpStatus,'trace_id'=>$trace,'context'=>Utils::redact($e->context)]);exit;}catch(\Throwable $e){if($started)exit;$trace=Utils::id('trace');try{Audit::record('delivery_unexpected_error',['trace_id'=>$trace,'exception_class'=>get_class($e)]);}catch(\Throwable){}http_response_code(500);header('Content-Type: application/json; charset=utf-8');header('Cache-Control: no-store');echo Utils::json(['code'=>'internal_error','message'=>'Delivery failed.','status'=>500,'trace_id'=>$trace,'context'=>[]]);exit;}finally{if(is_resource($stream))fclose($stream);}
    }
...
final class Rest {
    ...
    private static function assertContract($request): void {$expected=defined('SCM_CONTRACT_VERSION')?(string)SCM_CONTRACT_VERSION:'';$actual=self::header($request,'x-scm-contract-version');if($expected===''||$actual===''||!hash_equals($expected,$actual))throw new Error('contract_version_mismatch','X-SCM-Contract-Version is required and must match the active CF-04 contract.',409,['expected'=>$expected]);}
    private static function idempotencyKey($request): string {$key=self::header($request,'idempotency-key');$key=Utils::text($key,128);if($key==='')throw new Error('idempotency_key_required','Idempotency-Key header is required for this operation.',400);return $key;}
    ...
    public static function createUpload($request): mixed {return self::wrap(function()use($request){self::assertContract($request);$i=self::input($request);$result=UploadService::create(Auth::currentUser(),(array)($i['metadata']??[]),(array)($i['policy']??[]),self::idempotencyKey($request));$credential=(string)($result['upload_credential']??'');$view=self::clean($result,['parts','policy','rights']);$view['upload_credential']=$credential;return $view;});}
    public static function putPart($request): mixed {return self::wrap(function()use($request){self::assertContract($request);$id=self::route($request,'id');$part=(int)self::route($request,'part');$upload=RecordStore::get('upload',$id);if(!$upload||(int)($upload['actor_id']??0)!==Auth::currentUser())throw new Error('upload_not_found','Upload session not found.',404);$max=(int)($upload['policy']['max_part_size_bytes']??0);[$stream,$close]=self::partStream($request,$max);try{return self::clean(UploadService::putPart($id,Auth::currentUser(),$part,$stream,self::header($request,'x-content-sha256'),self::header($request,'x-upload-credential')));}finally{if($close&&is_resource($stream))fclose($stream);}});}
    public static function completeUpload($request): mixed {return self::wrap(function()use($request){self::assertContract($request);return self::clean(UploadService::complete(self::route($request,'id'),Auth::currentUser(),self::header($request,'x-upload-credential'),self::idempotencyKey($request)),['scan_results','job_graph']);});}
    public static function uploadAction($request): mixed {return self::wrap(function()use($request){self::assertContract($request);$i=self::input($request);$id=self::route($request,'id');$actor=Auth::currentUser();$cred=self::header($request,'x-upload-credential');return self::clean(match(self::route($request,'action')){'pause'=>UploadService::pause($id,$actor,$cred),'resume'=>UploadService::resume($id,$actor,$cred),'abort'=>UploadService::abort($id,$actor,$cred,(string)($i['reason']??'user-abort')),default=>throw new Error('upload_action_invalid','Invalid upload action.',400)},['policy','parts']);});}
    public static function processAsset($request): mixed {return self::wrap(function()use($request){self::assertContract($request);$id=self::route($request,'id');$actor=Auth::currentUser();Auth::assertActor($actor);$asset=RecordStore::get('asset',$id);if(!$asset)throw new Error('asset_not_found','Asset not found.',404);$decision=DomainRegistry::decision((string)$asset['owner_domain'],'authorize_processing',['asset'=>$asset,'actor_id'=>$actor,'operation'=>'start','channel'=>'rest']);if((int)$decision['object_version']!==(int)$asset['object_version'])throw new Error('domain_object_version_stale','Owner version changed.',409);$jobs=ProcessingService::start($id);return ['asset_id'=>$id,'processing_status'=>'queued','jobs'=>array_keys($jobs)];});}
    public static function issueGrant($request): mixed {return self::wrap(function()use($request){self::assertContract($request);$i=self::input($request);$actor=Auth::currentUser();$context=(array)($i['context']??[]);$context['audience_type']='private';$service=function_exists('apply_filters')?(string)apply_filters('scm_web_delivery_service_id','web',$actor):'web';$token=DeliveryService::issue((string)($i['asset_id']??''),isset($i['derivative_id'])?(string)$i['derivative_id']:null,$actor,$service,['type'=>'user','user_id'=>$actor],$context,(string)($i['operation']??'view'),(array)($i['range_policy']??[]),(string)($i['session_id']??''),(int)($i['ttl']??300),(int)($i['max_uses']??20));return ['endpoint'=>function_exists('home_url')?home_url('/scm-media-delivery/'):'/scm-media-delivery/','token'=>$token];});}
    public static function serveMetadata($request): mixed {return self::wrap(function()use($request){self::assertContract($request);$i=self::input($request);Utils::requireFields($i,['grant_id'],'grant_id_missing');$grant=RecordStore::get('grant',(string)$i['grant_id']);if(!$grant||(int)$grant['actor_id']!==Auth::currentUser())throw new Error('grant_not_found','Grant not found.',404);return ['grant_id'=>$grant['id'],'status'=>$grant['status'],'expires_at'=>$grant['expires_at'],'endpoint'=>function_exists('home_url')?home_url('/scm-media-delivery/'):'/scm-media-delivery/'];});}
    public static function createTransfer($request): mixed {return self::wrap(function()use($request){self::assertContract($request);$i=self::input($request);return self::clean(TransferService::create((array)($i['transfer']??[]),(array)($i['policy']??[])),['policy']);});}
    public static function listDownloads($request=null): mixed {return self::wrap(function()use($request){if($request!==null)self::assertContract($request);return array_map(fn($v)=>self::clean((array)$v),DownloadManagerService::list(Auth::currentUser()));});}
    public static function createDownload($request): mixed {return self::wrap(function()use($request){self::assertContract($request);$i=self::input($request);return self::clean(DownloadManagerService::create(Auth::currentUser(),(string)($i['asset_id']??''),isset($i['derivative_id'])?(string)$i['derivative_id']:null,(string)($i['mode']??'view'),(array)($i['context']??[])));});}
    public static function requestDeletion($request): mixed {return self::wrap(function()use($request){self::assertContract($request);$i=self::input($request);return self::clean(DeletionService::request((string)($i['asset_id']??''),Auth::currentUser(),(string)($i['reason']??'user-request'),(array)($i['context']??[])));});}
    public static function processDeletion($request): mixed {return self::wrap(function()use($request){self::assertContract($request);return self::clean(DeletionService::process(self::route($request,'id')));});}
    public static function placeHold($request): mixed {return self::wrap(function()use($request){self::assertContract($request);$i=self::input($request);return self::clean(LegalHoldService::place((string)($i['asset_id']??''),Auth::currentUser(),(array)($i['hold']??[])));});}
    public static function providerWebhook($request): mixed {return self::wrap(function()use($request){$provider=self::route($request,'provider');$body=self::bodyText($request,self::MAX_WEBHOOK_BYTES);$event=self::header($request,'x-event-id');$timestamp=(int)self::header($request,'x-event-timestamp');$signature=self::header($request,'x-event-signature');$secret=function_exists('apply_filters')?(string)apply_filters('scm_provider_webhook_secret','',$provider):'';return WebhookService::verify($provider,$event,$timestamp,$body,$signature,$secret);});}
    private static function wrap(callable $callback): mixed {try{$value=$callback();return function_exists('rest_ensure_response')?rest_ensure_response($value):$value;}catch(Error $e){$trace=Utils::id('trace');$context=Utils::redact($e->context);return class_exists('WP_Error')?new \WP_Error($e->errorCode,$e->getMessage(),['status'=>$e->httpStatus,'trace_id'=>$trace,'context'=>$context]):['code'=>$e->errorCode,'message'=>$e->getMessage(),'status'=>$e->httpStatus,'trace_id'=>$trace,'context'=>$context];}catch(\Throwable $e){$trace=Utils::id('trace');try{Audit::record('rest_unexpected_error',['trace_id'=>$trace,'exception_class'=>get_class($e)]);}catch(\Throwable){}$payload=['code'=>'internal_error','message'=>'The media service could not complete this request.','status'=>500,'trace_id'=>$trace,'context'=>[]];return class_exists('WP_Error')?new \WP_Error($payload['code'],$payload['message'],['status'=>500,'trace_id'=>$trace,'context'=>$payload['context']]):$payload;}}
}
