...
final class Rest {
    ...
    public static function processAsset($request): mixed {return self::wrap(function()use($request){$id=self::route($request,'id');$actor=Auth::currentUser();Auth::assertActor($actor);$asset=RecordStore::get('asset',$id);if(!$asset)throw new Error('asset_not_found','Asset not found.',404);$decision=DomainRegistry::decision((string)$asset['owner_domain'],'authorize_processing',['asset'=>$asset,'actor_id'=>$actor,'operation'=>'start','channel'=>'rest']);if((int)$decision['object_version']!==(int)$asset['object_version'])throw new Error('domain_object_version_stale','Owner version changed.',409);$jobs=ProcessingService::start($id);return ['asset_id'=>$id,'processing_status'=>'queued','jobs'=>array_keys($jobs)];});}
