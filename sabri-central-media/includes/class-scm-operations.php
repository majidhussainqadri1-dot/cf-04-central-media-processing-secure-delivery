...
final class ProviderExitService {
    ...
    foreach(RecordStore::all('asset',0,null,100000) as $asset){
        ...
        LegalHoldService::assertNoHold((string)$asset['id'],'provider_exit');ResidencyCryptoService::assertProviderRegion((string)$asset['id'],$targetProvider);
    ...
    foreach(RecordStore::all('derivative',0,null,200000) as $derivative){
        ...
        LegalHoldService::assertNoHold($assetId,'provider_exit');ResidencyCryptoService::assertProviderRegion($assetId,$targetProvider);
    ...
    public static function shadowVerify(string $exitId): array {
        ...
        try{foreach($plan['items'] as $item){$holdAssetId=($item['type']??'')==='asset'?(string)$item['id']:(string)($item['asset_id']??'');if($holdAssetId==='')throw new Error('provider_exit_parent_missing','Provider-exit item has no residency parent.',409,['id'=>$item['id']??'']);ResidencyCryptoService::assertProviderRegion($holdAssetId,(string)$plan['target_provider']);if(empty($item['target_key']))throw new Error('provider_exit_target_missing','Target mapping missing.',500,['id'=>$item['id']]);$stream=$target->openStream((string)$item['target_key']);try{$stats=Utils::streamHash($stream);}finally{fclose($stream);}if(!hash_equals((string)$item['sha256'],$stats['sha256'])||(int)$item['size']!==(int)$stats['size'])throw new Error('provider_shadow_mismatch','Shadow delivery integrity mismatch.',500,['id'=>$item['id']]);}$plan['status']='shadow_verified';$plan['shadow_verified_at']=Utils::now();return self::save($plan);}
    ...
    public static function rollback(string $exitId,int $actor,string $reason): array {
        ...
        $target=ProviderRegistry::get((string)$plan['target_provider']);foreach($plan['items'] as $index=>$item){if(($item['status']??'')!=='switched')continue;$holdAssetId=($item['type']??'')==='asset'?(string)$item['id']:(string)($item['asset_id']??'');if($holdAssetId==='')throw new Error('provider_exit_parent_missing','Provider-exit rollback item has no policy parent.',409,['id'=>$item['id']??'']);LegalHoldService::assertNoHold($holdAssetId,'provider_exit');ResidencyCryptoService::assertProviderRegion($holdAssetId,(string)$plan['source_provider']);$type=$item['type']==='asset'?'asset':'derivative';$record=RecordStore::get($type,(string)$item['id']);if(!$record||empty($record['previous_object_key'])||empty($record['previous_storage']))throw new Error('provider_exit_rollback_missing','Previous provider mapping missing.',500,['id'=>$item['id']]);$currentProvider=(string)($record['storage']['provider_id']??'');$currentKey=(string)($record['object_key']??'');if($currentProvider===(string)$plan['target_provider']&&$currentKey===(string)$item['target_key']){$record['object_key']=$record['previous_object_key'];$record['storage']=$record['previous_storage'];$record['provider_rollback_at']=Utils::now();RecordStore::put($type,(string)$record['id'],$record,(int)$record['version']);}elseif($currentProvider!==(string)$plan['source_provider']||$currentKey!==(string)$item['object_key'])throw new Error('provider_exit_rollback_mapping_stale','Provider mapping changed during rollback.',409,['id'=>$item['id']]);if(ResidencyCryptoService::isLocked($holdAssetId)){$plan['items'][$index]['target_cleanup_deferred']='object_lock';}elseif($target->exists((string)$item['target_key'])&&!$target->delete((string)$item['target_key'])&&$target->exists((string)$item['target_key']))throw new Error('provider_exit_rollback_cleanup_failed','Rollback target cleanup failed.',503,['id'=>$item['id']]);$plan['items'][$index]['status']='verified';$plan=self::save(self::counted($plan));}ProviderRegistry::activate((string)$plan['source_provider']);$plan['status']='rolled_back';$plan['rollback_by']=$actor;$plan['rollback_reason']=$reason;$plan['rolled_back_at']=Utils::now();return self::save(self::counted($plan));
    }
    public static function purgeSource(string $exitId): array {
        $plan=self::load($exitId,['switched','purge_pending','credentials_pending']);$source=ProviderRegistry::get((string)$plan['source_provider']);$plan['status']='purge_pending';$plan=self::save($plan);
        foreach($plan['items'] as $index=>$item){if(($item['status']??'')==='purged')continue;if(($item['status']??'')!=='switched')throw new Error('provider_exit_item_not_switched','Provider exit item was not switched.',409,['id'=>$item['id']]);$holdAssetId=($item['type']??'')==='asset'?(string)$item['id']:(string)($item['asset_id']??'');if($holdAssetId==='')throw new Error('provider_exit_parent_missing','Provider-exit item has no hold parent.',409,['id'=>$item['id']??'']);LegalHoldService::assertNoHold($holdAssetId,'provider_exit');ResidencyCryptoService::assertUnlocked($holdAssetId,'provider_exit_purge');$type=$item['type']==='asset'?'asset':'derivative';$record=RecordStore::get($type,(string)$item['id']);if(!$record||($record['storage']['provider_id']??'')!==(string)$plan['target_provider']||($record['object_key']??'')!==(string)$item['target_key'])throw new Error('provider_exit_target_mapping_invalid','Target mapping is not authoritative.',409,['id'=>$item['id']]);if($source->exists((string)$item['object_key'])&&!$source->delete((string)$item['object_key'])&&$source->exists((string)$item['object_key']))throw new Error('provider_source_purge_failed','Source provider purge failed.',503,['id'=>$item['id']]);$plan['items'][$index]['status']='purged';$plan=self::save(self::counted($plan));}
        $plan['status']='credentials_pending';$plan['credentials_revocation_required']=true;$plan['source_purge_completed_at']=$plan['source_purge_completed_at']??Utils::now();$plan=self::save(self::counted($plan));return self::finalizeCredentials($plan);
    }
    private static function finalizeCredentials(array $plan): array {
        $source=ProviderRegistry::get((string)$plan['source_provider']);
        if(!($source instanceof CredentialRevocableObjectStore)){throw new Error('provider_credentials_revocation_unavailable','Source provider cannot prove credential revocation; provider exit remains pending.',503,['provider'=>$plan['source_provider']]);}
        $evidence=$source->revokeExitCredentials(['exit_id'=>$plan['id'],'source_provider'=>$plan['source_provider'],'target_provider'=>$plan['target_provider'],'purged'=>(int)($plan['purged']??0)]);
        if(($evidence['revoked']??false)!==true){throw new Error('provider_credentials_revocation_unconfirmed','Source-provider credential revocation was not confirmed.',503);}
        $fresh=RecordStore::get('provider_exit',(string)$plan['id'])??$plan;$fresh['credentials_revocation_required']=false;$fresh['credentials_revocation_evidence']=Utils::redact($evidence);$fresh['credentials_revoked_at']=Utils::now();$fresh['status']='completed';$fresh['completed_at']=Utils::now();return self::save(self::counted($fresh));
    }
    private static function assertSourceDrained(array $plan): void {
...
final class KeyRotationService {
    ...
    foreach($groups as $groupKey=>$group){
        ...
        try{
            ...
            if($locked){$result['locked_old_objects_retained']++;self::deferCleanup($runId,$providerId,$oldKey,$group);}elseif($provider->exists($oldKey)&&!$provider->delete($oldKey)&&$provider->exists($oldKey))throw new Error('key_rotation_cleanup_failed','Old encrypted object could not be deleted.',503);
            $result['groups']++;
...
final class RepairService {
    ...
    $asset=RecordStore::get('asset',(string)$repair['asset_id']);if(!$asset)throw new Error('asset_not_found','Asset not found.',404);
    LegalHoldService::assertNoHold((string)$asset['id'],'reprocess');
    $decision=DomainRegistry::decision($asset['owner_domain'],'authorize_reprocess',['asset'=>$asset,'actor_id'=>$actor,'reason'=>'rollback:'.(string)$repair['reason'],'target_kinds'=>$repair['target_kinds']??[],'preset'=>$repair['preset']??[],'rollback_to_manifest'=>$repair['previous_manifest_id']]);
...
final class RestoreService {
    ...
    private static function manifests(): bool {foreach(RecordStore::all('asset',0,null,100000) as $a){if(($a['status']??'')!=='ready')continue;$manifestId=(string)($a['active_manifest_id']??'');if($manifestId==='')return false;$m=RecordStore::get('manifest',$manifestId);if(!$m||($m['status']??'')!=='active'||($m['asset_id']??'')!==$a['id'])return false;$items=(array)($m['derivatives']??[]);if($items===[])return false;foreach($items as $item){$id=(string)($item['derivative_id']??'');$d=$id!==''?RecordStore::get('derivative',$id):null;if(!$d||($d['asset_id']??'')!==$a['id']||($d['status']??'')!=='validated'||!empty($d['superseded_by']))return false;if(!hash_equals((string)($item['sha256']??''),(string)($d['sha256']??''))||(int)($item['size']??-1)!==(int)($d['size']??-2)||(string)($item['kind']??'')!==(string)($d['kind']??''))return false;}}return true;}
    ...
    if(!$restore||!$gate||($gate['restore_id']??'')!==$restoreId)throw new Error('restore_gate_blocked','Restore reconciliation has not passed.',503);
    ...
    public static function assertServeAllowed(): void {
        $gate=RecordStore::get('restore_gate','current');
        if($gate&&!in_array(($gate['status']??''),['serve_authorized','cancelled'],true))throw new Error('restore_gate_blocked','Media serving is blocked during restore reconciliation.',503,['restore_id'=>$gate['restore_id']??'']);
    }
