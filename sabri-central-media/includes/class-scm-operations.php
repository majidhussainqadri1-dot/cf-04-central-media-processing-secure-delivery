...
final class RestoreService {
    ...
    private static function manifests(): bool {foreach(RecordStore::all('asset',0,null,100000) as $a){if(($a['status']??'')!=='ready')continue;$manifestId=(string)($a['active_manifest_id']??'');if($manifestId==='')return false;$m=RecordStore::get('manifest',$manifestId);if(!$m||($m['status']??'')!=='active'||($m['asset_id']??'')!==$a['id']||(int)($m['processing_generation']??0)!==(int)($a['processing_generation']??0))return false;$items=(array)($m['derivatives']??[]);if($items===[])return false;foreach($items as $item){$id=(string)($item['derivative_id']??'');$d=$id!==''?RecordStore::get('derivative',$id):null;if(!$d||($d['asset_id']??'')!==$a['id']||($d['status']??'')!=='validated'||!empty($d['superseded_by']))return false;if(!hash_equals((string)($item['sha256']??''),(string)($d['sha256']??''))||(int)($item['size']??-1)!==(int)($d['size']??-2)||(string)($item['kind']??'')!==(string)($d['kind']??''))return false;}}return true;}
