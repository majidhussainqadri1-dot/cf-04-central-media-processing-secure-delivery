...
final class Idempotency {
    ...
    public static function claim(string $scope,string $key,string $fingerprint,int $ttl=86400): array {
        ...
        $row=['actor_id'=>$actor,'scope'=>$scope,'fingerprint'=>$fingerprint,'status'=>'claimed','expires_at'=>Utils::now()+$ttl,'claimed_at'=>Utils::now()];
        return ['replay'=>false,'record'=>RecordStore::put('idempotency',$id,$row,$record?(int)$record['version']:0)];
    }
    public static function complete(string $scope,string $key,string $fingerprint,string $resultType,string $resultId,array $result=[]): array {
        [,,$actor,$id]=self::identity($scope,$key);$record=RecordStore::get('idempotency',$id);
        if(!$record||(int)($record['actor_id']??0)!==$actor||!hash_equals((string)$record['fingerprint'],$fingerprint))throw new Error('idempotency_claim_missing','Idempotency claim missing.',409);
    ...
    public static function fail(string $scope,string $key,string $fingerprint,string $code): void {
        try{[,,$actor,$id]=self::identity($scope,$key);}catch(Error){return;}$r=RecordStore::get('idempotency',$id);
        if(!$r||(int)($r['actor_id']??0)!==$actor||!hash_equals((string)$r['fingerprint'],$fingerprint))return;
...
final class PartStore {
    public static function put(string $uploadId,int $part,$stream,string $expectedHash): array {
        ...
        try{return RecordStore::put('upload_part',$id,['actor_id'=>Auth::currentUser(),'upload_id'=>$uploadId,'part_number'=>$part,'provider_id'=>ProviderRegistry::activeId(),'object_key'=>$stored['object_key'],'size'=>$hash['size'],'sha256'=>$expectedHash,'status'=>'stored','created_at'=>Utils::now()]);}
...
final class UploadService {
    ...
    public static function create(int $actor,array $metadata,array $rawPolicy,string $idempotencyKey,array $quotaLimits=[]): array {
        ...
        $policy=Policy::normalize($rawPolicy,true);self::validateMetadata($metadata,$policy);$fingerprint=hash('sha256',Utils::canonicalJson(['actor'=>$actor,'metadata'=>Utils::redact($metadata),'policy_hash'=>$policy['policy_hash']]));$claim=Idempotency::claim('upload-create',$idempotencyKey,$fingerprint);
        ...
        $ownerContext=['actor_id'=>$actor,'owner_object'=>Utils::text((string)($metadata['owner_object']??''),191),'policy'=>$policy,'metadata'=>Utils::redact($metadata)];$decision=DomainRegistry::decision($policy['owner_domain'],'authorize_upload',$ownerContext);
    ...
    public static function putPart(string $uploadId,int $actor,int $part,$stream,string $sha256,string $credential): array {RuntimeGuard::requireReady(['streaming']);$upload=RecordStore::get('upload',$uploadId);if(!$upload)throw new Error('upload_not_found','Upload session not found.',404);self::authenticate($upload,$actor,$credential);if(($upload['status']??'')!=='uploading')throw new Error('upload_state_invalid','Upload is not accepting parts.',409);if($part<1||$part>(int)$upload['policy']['max_upload_parts'])throw new Error('part_number_invalid','Part number outside policy.',400);$hash=Utils::streamHash($stream);if($hash['size']<1||$hash['size']>(int)$upload['policy']['max_part_size_bytes'])throw new Error('part_size_exceeded','Part exceeds policy.',413);if($hash['size']+(int)$upload['received_size']>(int)$upload['expected_size']+(int)$upload['policy']['max_part_size_bytes'])throw new Error('upload_size_exceeded','Uploaded parts exceed expected bounds.',422);$stored=PartStore::put($uploadId,$part,$stream,strtolower($sha256));$parts=PartStore::list($uploadId);$upload['received_size']=array_sum(array_map(fn($p)=>(int)$p['size'],$parts));$upload['received_parts']=count($parts);$upload['progress_percent']=(float)min(100.0,round(100*$upload['received_size']/max(1,(int)$upload['expected_size']),2));$upload=RecordStore::put('upload',$uploadId,$upload,(int)$upload['version']);return ['upload_id'=>$uploadId,'part'=>$stored,'received_size'=>$upload['received_size'],'progress_percent'=>$upload['progress_percent']];}
    public static function pause(string $uploadId,int $actor,string $credential): array {$u=RecordStore::get('upload',$uploadId);if(!$u)throw new Error('upload_not_found','Upload not found.',404);self::authenticate($u,$actor,$credential);if(($u['status']??'')!=='uploading')throw new Error('upload_state_invalid','Upload cannot be paused.',409);$u['status']='paused';return RecordStore::put('upload',$uploadId,$u,(int)$u['version']);}
    public static function resume(string $uploadId,int $actor,string $credential): array {$u=RecordStore::get('upload',$uploadId);if(!$u)throw new Error('upload_not_found','Upload not found.',404);self::authenticate($u,$actor,$credential);if(($u['status']??'')!=='paused')throw new Error('upload_state_invalid','Upload cannot be resumed.',409);$u['status']='uploading';return RecordStore::put('upload',$uploadId,$u,(int)$u['version']);}
    ...
    public static function complete(string $uploadId,int $actor,string $credential,string $idempotencyKey): array {
        RuntimeGuard::requireReady(['streaming']);$u=RecordStore::get('upload',$uploadId);if(!$u)throw new Error('upload_not_found','Upload not found.',404);self::authenticate($u,$actor,$credential);if(($u['status']??'')!=='uploading')throw new Error('upload_state_invalid','Upload cannot be completed.',409);$fingerprint=hash('sha256',$uploadId.'|'.$u['expected_sha256'].'|'.$u['expected_size']);$claim=Idempotency::claim('upload-complete',$idempotencyKey,$fingerprint);if($claim['replay'])return RecordStore::get('asset',(string)$claim['record']['result_id'])??throw new Error('asset_replay_missing','Completed asset missing.',500);
