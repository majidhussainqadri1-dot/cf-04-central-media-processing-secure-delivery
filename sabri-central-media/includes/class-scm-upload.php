...
final class UploadService {
    ...
    public static function putPart(string $uploadId,int $actor,int $part,$stream,string $sha256,string $credential): array {RuntimeGuard::requireReady(['streaming']);$upload=RecordStore::get('upload',$uploadId);if(!$upload)throw new Error('upload_not_found','Upload session not found.',404);self::authenticate($upload,$actor,$credential);if(($upload['status']??'')!=='uploading')throw new Error('upload_state_invalid','Upload is not accepting parts.',409);if($part<1||$part>(int)$upload['policy']['max_upload_parts'])throw new Error('part_number_invalid','Part number outside policy.',400);$hash=Utils::streamHash($stream);if($hash['size']<1||$hash['size']>(int)$upload['policy']['max_part_size_bytes'])throw new Error('part_size_exceeded','Part exceeds policy.',413);$existingPart=null;foreach(PartStore::list($uploadId) as $candidate)if((int)($candidate['part_number']??0)===$part){$existingPart=$candidate;break;}$projected=(int)$upload['received_size']-(int)($existingPart['size']??0)+(int)$hash['size'];if($projected>(int)$upload['expected_size'])throw new Error('upload_size_exceeded','Uploaded parts exceed expected size.',422);$stored=PartStore::put($uploadId,$part,$stream,strtolower($sha256));$parts=PartStore::list($uploadId);$upload['received_size']=array_sum(array_map(fn($p)=>(int)$p['size'],$parts));$upload['received_parts']=count($parts);$upload['progress_percent']=(float)min(100.0,round(100*$upload['received_size']/max(1,(int)$upload['expected_size']),2));$upload=RecordStore::put('upload',$uploadId,$upload,(int)$upload['version']);return ['upload_id'=>$uploadId,'part'=>$stored,'received_size'=>$upload['received_size'],'progress_percent'=>$upload['progress_percent']];}
    ...
    public static function complete(string $uploadId,int $actor,string $credential,string $idempotencyKey): array {
        RuntimeGuard::requireReady(['streaming']);$u=RecordStore::get('upload',$uploadId);if(!$u)throw new Error('upload_not_found','Upload not found.',404);self::authenticate($u,$actor,$credential);if(!in_array(($u['status']??''),['uploading','finalizing','completed'],true))throw new Error('upload_state_invalid','Upload cannot be completed.',409);$fingerprint=hash('sha256',$uploadId.'|'.$u['expected_sha256'].'|'.$u['expected_size']);$claim=Idempotency::claim('upload-complete',$idempotencyKey,$fingerprint);if($claim['replay'])return RecordStore::get('asset',(string)$claim['record']['result_id'])??throw new Error('asset_replay_missing','Completed asset missing.',500);
