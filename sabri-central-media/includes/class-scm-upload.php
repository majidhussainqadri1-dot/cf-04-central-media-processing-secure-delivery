...
final class UploadService {
    ...
    public static function resume(string $uploadId,int $actor,string $credential): array {RuntimeGuard::requireReady(['streaming']);$u=RecordStore::get('upload',$uploadId);if(!$u)throw new Error('upload_not_found','Upload not found.',404);self::authenticate($u,$actor,$credential);if(($u['status']??'')!=='paused')throw new Error('upload_state_invalid','Upload cannot be resumed.',409);$u['status']='uploading';return RecordStore::put('upload',$uploadId,$u,(int)$u['version']);}
