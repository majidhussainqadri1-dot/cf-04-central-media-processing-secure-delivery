...
final class PerceptualMediaService {
    ...
    public static function dedupeDecision(string $assetId,string $candidateId,int $actor): array {
        ...
        $allowed=$asset['sha256']===$candidate['sha256']&&Future40Registry::sameEnvelope($asset,$candidate)&&($asset['retention_class']??null)===($candidate['retention_class']??null);
...
final class ContentSafetyUpgradeService {
    ...
    public static function scheduleRescan(string $reason,array $filters=[],int $limit=1000): array {
        $reason=Utils::key($reason,96);if($reason==='')throw new Error('rescan_reason_required','Re-scan reason is required.',400);$limit=max(1,min(5000,$limit));$queued=0;
        foreach(RecordStore::all('asset',0,null,$limit) as $asset){if(in_array(($asset['status']??''),['deleted','rejected'],true))continue;if(isset($filters['mime'])&&$asset['mime']!==$filters['mime'])continue;if(isset($filters['media_class'])&&$asset['media_class']!==$filters['media_class'])continue;
            $id=hash('sha256',$asset['id'].'|'.$reason);if(RecordStore::get('future_rescan',$id))continue;RecordStore::put('future_rescan',$id,['actor_id'=>0,'status'=>'queued','asset_id'=>$asset['id'],'reason'=>$reason,'requested_at'=>Utils::now()]);$queued++;}
        Audit::record('future40_rescan_scheduled',['reason'=>$reason,'queued'=>$queued]);return ['queued'=>$queued,'reason'=>$reason];
    }
...
final class OperationsFutureService {
    ...
    public static function migrationBundle(array $assetIds,int $actor,string $target): array {RuntimeGuard::requireReady();Auth::assertActor($actor,'media_manage_providers');$target=Utils::key($target,64);if($target===''||$assetIds===[])throw new Error('migration_bundle_invalid','Target and assets are required.',400);$items=[];foreach(array_values(array_unique($assetIds)) as $assetId){$asset=Future40Registry::asset((string)$assetId);$items[]=['asset_id'=>$asset['id'],'sha256'=>$asset['sha256'],'privacy_class'=>$asset['privacy_class'],'policy_hash'=>$asset['policy_hash'],'rights_hash'=>$asset['rights']['policy_hash'],'status'=>$asset['status'],'object_version'=>$asset['object_version']];}$bundle=['format'=>'SCM-MIGRATION-1','target'=>$target,'created_at'=>Utils::now(),'items'=>$items];$payload=Utils::canonicalJson($bundle);$bundle['signature']=hash_hmac('sha256',$payload,Keyring::hashKey());$bundle['bundle_hash']=hash('sha256',$payload);$id=Utils::id('migration');RecordStore::put('migration_bundle',$id,['actor_id'=>$actor,'status'=>'signed','target'=>$target,'bundle_hash'=>$bundle['bundle_hash'],'signature'=>$bundle['signature'],'item_count'=>count($items),'created_at'=>Utils::now()]);return ['bundle_id'=>$id,'bundle'=>$bundle];}
    public static function verifyMigrationBundle(array $bundle): bool {$signature=(string)($bundle['signature']??'');unset($bundle['signature'],$bundle['bundle_hash']);return $signature!==''&&hash_equals(hash_hmac('sha256',Utils::canonicalJson($bundle),Keyring::hashKey()),$signature);}
