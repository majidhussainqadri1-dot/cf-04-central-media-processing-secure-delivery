...
final class Utils {
    ...
    public static function streamHash($stream,?int $maxBytes=null): array { if(!is_resource($stream))throw new Error('stream_invalid','Invalid stream.',500);if($maxBytes!==null&&$maxBytes<1)throw new Error('stream_limit_invalid','Stream limit must be positive.',500); rewind($stream); $ctx=hash_init('sha256');$size=0;while(!feof($stream)){ $chunk=fread($stream,1048576); if($chunk===false)throw new Error('stream_read_failed','Stream read failed.',500);if($chunk===''&&!feof($stream))throw new Error('stream_stalled','Stream produced no data before EOF.',500);$size+=strlen($chunk);if($maxBytes!==null&&$size>$maxBytes){rewind($stream);throw new Error('stream_size_exceeded','Stream exceeds allowed size.',413,['max_bytes'=>$maxBytes]);}hash_update($ctx,$chunk);}rewind($stream);return ['sha256'=>hash_final($ctx),'size'=>$size]; }
