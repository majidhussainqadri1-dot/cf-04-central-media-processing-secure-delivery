...
r73(str_contains($s,"['uploading','paused','aborting']"),'abort has an explicit retryable intermediate state');
r73(str_contains($s,"$u['status']='aborting'"),'abort blocks further upload writes before destructive cleanup starts');
$purge=strpos($s,'PartStore::purge($uploadId)');$final=strpos($s,"$fresh['status']='aborted'");
r73($purge!==false&&$final!==false&&$purge<$final,'final aborted status is persisted only after part purge is attempted');
$settle=strpos($s,"QuotaService::settle((string)$u['quota']['quota_id']");
r73($settle!==false&&$settle<$final,'quota release precedes final aborted status');
r73(str_contains($s,"if($state==='aborted')return $u"),'already-finalized abort remains idempotent');
