...
r82(str_contains($s,"status']??'')!=='claimed'")&&str_contains($s,"record['claim_token']??''"),'completion requires the currently claimed generation');
r82(str_contains($s,"r['claim_token']??''"),'failure cannot overwrite a newer reclaimed generation');
