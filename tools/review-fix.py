...
p=ROOT/'sabri-central-media/includes/class-scm-delivery.php';s=p.read_text()
...
start=s.find("    $existing=array_values(array_filter(RecordStore::all('cdn_mapping'")
end=s.find("    return RecordStore::put('cdn_mapping',Utils::id('cdn'),$map);",start)
if start<0 or end<0: raise SystemExit('round 88 CDN publication anchor missing')
end += len("    return RecordStore::put('cdn_mapping',Utils::id('cdn'),$map);")
block=s[start:end]
cache="    $cacheKey=hash('sha256',$assetId.'|'.$derivativeId.'|'.$derivative['sha256'].'|'.$asset['privacy_class'].'|'.$asset['policy_hash'].'|'.$asset['rights']['policy_hash']).'/'.$derivative['sha256'];"
if cache not in block: raise SystemExit('round 88 cache-key anchor missing')
replacement="""    $existing=array_values(array_filter(RecordStore::all('cdn_mapping',0,null,100000),fn($mapping)=>($mapping['asset_id']??'')===$assetId&&($mapping['derivative_id']??'')===$derivativeId&&($mapping['status']??'')==='published'&&($mapping['sha256']??'')===$derivative['sha256']&&($mapping['privacy_class']??'')===$asset['privacy_class']&&hash_equals((string)($mapping['policy_hash']??''),(string)$asset['policy_hash'])&&hash_equals((string)($mapping['rights_hash']??''),(string)$asset['rights']['policy_hash'])));
...
t=ROOT/'tests/review-round-88-delivery-cdn-atomicity.php';t.write_text(r'''<?php
...
''')
q=ROOT/'tools/quality-check.sh';x=q.read_text();a='php "$ROOT/tests/review-round-87-storage-boundaries.php"\n';line='php "$ROOT/tests/review-round-88-delivery-cdn-atomicity.php"\n'
if line not in x:
    if a not in x: raise SystemExit('round 88 quality anchor missing')
    q.write_text(x.replace(a,a+line,1))
