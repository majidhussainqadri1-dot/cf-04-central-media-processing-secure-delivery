...
raw_uuid=hashlib.sha256((version+'CF-04').encode()).hexdigest()[:32]
uuid=f"{raw_uuid[:8]}-{raw_uuid[8:12]}-{raw_uuid[12:16]}-{raw_uuid[16:20]}-{raw_uuid[20:32]}"
sbom={'bomFormat':'CycloneDX','specVersion':'1.5','serialNumber':'urn:uuid:'+uuid,'version':1,'metadata':{'component':{'type':'application','name':'cf-04-sabri-central-media','version':version}},'components':[{'type':'file','name':x['path'],'hashes':[{'alg':'SHA-256','content':x['sha256']}]} for x in files]}
